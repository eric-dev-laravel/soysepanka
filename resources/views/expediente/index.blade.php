		...
		*/
		...
		/* ----------------------------------------
			...
			*/
			...
			/********************************************************************************************/
			/*************************** vita pestaña beneficiarios *************************************/
			/* ----------------------------------------
				* Agregar renglones a examenes medicos cada que se presiona
				* el botón agregar y mostrarlos debajo
				* y también la función de eliminar el renglón
				* -------------------------------------------
			*/
				...
				/* validamos si existen el perfil y la relacion 
				* pintamos los datos provenientes de la base de datos
				* asignamos el mismo arreglo por si se agregan/eliminan filas
				*/
				@isset($data['record_beneficiaries_det'])
					detalle = {!! json_encode($data['record_beneficiaries_det']) !!};
					var con1 = 0;
					$.each(detalle, function(i, item) {
						inputNameBenefit = '<div class="form-group col-md-4"><input type="text" name="beneficiarie_name_son[]" class="col-2 form-control" value="'+item.beneficiarie_name+'" readonly="true"></div>';
						inputDateBenefit = '<div class="form-group col-md-2"><input type="text" name="date_birth_beneficiarie_son[]" class="col-2 form-control" value="'+item.date_birth_beneficiarie+'" readonly="true"></div>';
						inputFileM = '<div class="form-group col-md-4"><input type="text" name="file_birth_certificate_beneficiarie_son[]" class="col-2 form-control" value="'+item.file_birth_certificate_beneficiarie+'" readonly="true"></div>';
						deleteRow = '<div class="form-group col-md-1"><button type="button" class="col-1 btn btn-danger" id="borrar"><span class="fa fa-minus"></span></button></div>';
						if(item.file_birth_certificate_beneficiarie != null) {
							viewFile = "<div class='form-group col-md-1'><a type='button' class='btn btn-primary' href='{!! asset('/storage/"+item.file_birth_certificate_beneficiarie+"') !!}' target='_blank'><i class='fa fa-file-alt'></i> Ver </a></div>";
							$('#destino_beneficiarios_hijos').append('<div class="row">'+inputNameBenefit+inputDateBenefit+inputFileM+viewFile+deleteRow+'</div>');
						} else {
							viewFile = "<div class='form-group col-md-1'></div>";
							$('#destino_beneficiarios_hijos').append('<div class="row">'+inputNameBenefit+inputDateBenefit+inputFileM+viewFile+deleteRow+'</div>');
						}
						con1++;
					});
				...
				$('#add_sons').on('click', function(e) {
					e.preventDefault();     
					
					benefit_name.push($('#beneficiarie_name_son').val());
					benefit_date.push($('#date_birth_beneficiarie_son').val());
					
					var lastNameBenefit = benefit_name[benefit_name.length-1];
					var lastDateBenefit = benefit_date[benefit_date.length-1];
					
					inputNameBenefit = '<div class="form-group col-md-4"><input type="text" name="beneficiarie_name_son[]" class="form-control" value="'+lastNameBenefit+'" readonly="true"></div>';
					inputDateBenefit = '<div class="form-group col-md-2"><input type="text" name="date_birth_beneficiarie_son[]" class="form-control" value="'+lastDateBenefit+'" readonly="true"></div>';
					inputFileBe = '<div class="form-group col-md-5"><input type="text" name="benefit_proof'+con_benefit+'" id="benefit_proof'+con_benefit+'" class="form-control" readonly="true"> <span id="benefit2_proof'+con_benefit+'"><input type="file" id="benefit2_proof'+con_benefit+' name="benefit2_proof'+con_benefit+'" style="display:none"/></div>';
					deleteRow = '<div class="form-group col-md-1"><button type="button" class="btn btn-danger" id="borrar"><span class="fa fa-minus"></span></button></div>';
					$('#destino_beneficiarios_hijos').append('<div class="row">'+inputNameBenefit+inputDateBenefit+inputFileBe+deleteRow+'</div>');
					//file_birth_certificate_beneficiarie
					clone_benefit = $("#file_birth_certificate_beneficiarie_son").clone();
					clone_benefit.attr('id', 'benefit2_proof'+con_benefit);
					$("#benefit_proof"+con_benefit).html(clone_benefit);
					$("#benefit_proof"+con_benefit).val(clone_benefit[0]['files'][0]['name']);

					$('#beneficiarie_name_son').val('');
					$("#date_birth_beneficiarie_son").val('');
					$("#file_birth_certificate_beneficiarie_son").val('');
					con_benefit++;
				});

				/* ----------------------------------------
				* eliminar renglón de referencias personales
				* ----------------------------------------
				*/
				$("#destino_beneficiarios_hijos").on('click', '#borrar',function(e) {
					e.preventDefault();
					$(this).parent().parent().remove();
				});
